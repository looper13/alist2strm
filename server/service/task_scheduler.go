...
func (s *TaskScheduler) AddTask(t *task.Task) error {
	...
	taskFunc := func() {
		...
		go func() {
			...
			if !currentTask.Enabled || currentTask.Running {
				return
			}
			...
			if taskQueue.IsTaskInQueue(t.ID) {
				utils.Info("任务已在队列中，跳过本次调度", "task_id", t.ID)
				return
			}

			// 将任务添加到队列
			utils.Info("定时任务触发，添加到执行队列", "task_id", t.ID, "name", currentTask.Name)
	...
	utils.Info("任务已添加到调度器", "task_id", t.ID, "cron", t.Cron)
	return nil
}
