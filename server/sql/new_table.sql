...
CREATE TABLE `file_histories` (
    ...
    `sourceUrl` VARCHAR(512),                          -- 源文件的完整访问URL（包含签名）
    ...
    -- 文件分类和关联
    `fileCategory` VARCHAR(50) NOT NULL DEFAULT 'main',  -- 文件类别：main/metadata/subtitle
    `isMainFile` TINYINT(1) NOT NULL DEFAULT 1,          -- 是否为主文件（视频文件）
    `mainFileId` INTEGER,                                -- 关联的主文件ID（如果是刮削数据或字幕文件）
    
    -- 处理状态管理
    `processingStatus` VARCHAR(50) NOT NULL DEFAULT 'success',  -- 处理状态：success/failed/pending/skipped
    `processingMessage` TEXT,                                   -- 处理消息或错误信息
    `retryCount` INTEGER NOT NULL DEFAULT 0,                   -- 重试次数
    `lastProcessedAt` DATETIME,                                 -- 最后处理时间
    
    -- 失效检测相关
    `lastCheckedAt` DATETIME,                          -- 最后检查时间（用于失效检测）
    `isValid` TINYINT(1) DEFAULT 1,                    -- 文件是否有效（失效检测结果）
    `validationMessage` TEXT,                          -- 验证失败的原因
    `validationRetryCount` INTEGER NOT NULL DEFAULT 0, -- 验证重试次数
    `nextCheckAt` DATETIME,                            -- 下次检查时间
    
    -- 通知相关
    `notificationStatus` INTEGER NOT NULL DEFAULT 0,   -- 通知状态：0=未发送,1=成功通知,2=失败通知,3=失效通知
    `embyNotified` TINYINT(1) NOT NULL DEFAULT 0,      -- Emby 通知状态
    `telegramNotified` TINYINT(1) NOT NULL DEFAULT 0,  -- Telegram 通知状态
    `notificationSentAt` DATETIME,                     -- 通知发送时间
    `notificationMessage` TEXT,                        -- 通知消息内容
    
    -- 扩展字段
    `mediaInfo` TEXT,                                  -- 媒体信息（JSON格式，可选）
    ...
    INDEX `idx_file_category` (`fileCategory`),
    INDEX `idx_main_file_id` (`mainFileId`),
    INDEX `idx_processing_status` (`processingStatus`),
    INDEX `idx_last_checked` (`lastCheckedAt`),
    INDEX `idx_is_valid` (`isValid`),
    INDEX `idx_notification_status` (`notificationStatus`),
    INDEX `idx_next_check_at` (`nextCheckAt`),
...
CREATE TABLE `notification_queue` (
    ...
    `createdAt` DATETIME,
    `updatedAt` DATETIME,
    `type` VARCHAR(50) NOT NULL,                       -- 通知类型：emby/telegram
    `event` VARCHAR(100) NOT NULL,                     -- 事件类型：task_completed/task_failed/file_invalid
